...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username field with the school user, then fill the password, then submit the login form (perform login). Immediately after login, wait for the app to load before continuing to settings.
        ...
        # -> Open the Settings view so we can modify a settings field.
        await page.goto("http://127.0.0.1:5176/settings")
        
        # -> Edit the School Name field to a new value, then scroll down to reveal the Save/Submit button so we can save the change.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/main/div/div/div[3]/form/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('NANJATI CDSS (UPDATED)')
        
        # -> Click the 'Save Settings' button to persist the change, then wait for the UI to reflect the saved state.
        frame = context.pages[-1]
        # Click element
