...
async def run_test():
    ...
    try:
        ...
        # -> Enter the username into the username field (index 37).
        ...
        # -> Trigger logout (use the logout route), then return to the login page and attempt to sign in again with username NANJATICDSS and password 1994.
        await page.goto("http://127.0.0.1:5176/logout")
        
        await page.goto("http://127.0.0.1:5176/login")
        
        # -> Fill the username and password fields and submit the login form to sign in again (use username NANJATICDSS and password 1994).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div[2]/form/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('NANJATICDSS')
